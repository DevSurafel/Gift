...
# Get the API_TOKEN and CHAT_ID from environment variables
API_TOKEN = os.getenv('API_TOKEN')  # Fetch API_TOKEN from the environment
CHAT_ID = os.getenv('CHAT_ID')      # Fetch CHAT_ID from the environment

if not API_TOKEN or not CHAT_ID:
    raise ValueError("API_TOKEN or CHAT_ID not set in environment variables.")
...
async def send_transaction_histories():
    ...
    while True:
        for transaction, markup in transaction_histories:
            await bot.send_message(chat_id=CHAT_ID, text=transaction, reply_markup=markup)
...
async def start(update, context):
    await update.message.reply_text("Bot started, sending transactions...")
...
def main():
    ...
    # Run the asynchronous transaction sending task
    asyncio.run(send_transaction_histories())
